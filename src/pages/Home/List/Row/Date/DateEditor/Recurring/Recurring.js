...
export const Recurring = ({recurringEnabled, recurring, setRecurring, childrenRef, defaultWeekdays, startDate}) => {
        ...
        <div className='flex flex-col'>
          ...
          <div className='flex flex-row items-center justify-center'>
            {recurring.weekdays.map((weekday, index) =>
              <button 
                key={index}
                className={'m-1 h-8 w-8 rounded-full text-center text-xs ' + (weekday ? 'bg-[#1677ff] text-white' : 'bg-gray-100')}
                onClick={() => {
                  let newWeekdays = recurring.weekdays
                  newWeekdays[index] = !newWeekdays[index]
                  if (!newWeekdays.includes(true)) newWeekdays = defaultWeekdays
                  setRecurring({
                    ...recurring,
              ...
              >
                {DateTime.now().startOf('week').plus({days: index - 1}).toFormat('ccccc')}
              </button>
            )}
          </div>
        </div>
          ...
          <div className='flex flex-row my-1 items-center justify-center'>
            ...
            >
              <DatePicker
                ...
                onSelect={(value) => setRecurring({
                  ...recurring,
                  ends_on: value
                })}
                onOk={(value) => setRecurring({
                  ...recurring,
                  ends_on: value
                })}
