...
export const useListCalendars = () => {
  ...
  useEffect(() => {
    if (calendars === null) getCalendars();
  }, []);
  return calendars;
};
...
export const listEvents = async (calendarID) => {
  ...
  let items = recurring_events.map(({ ...rest }) => {
    ...
    let start_date = DateTime.fromISO(
      start_date_or_time ? rest.start.date : rest.start.dateTime
    );
    let end_date_or_time = rest.end.date ? true : false;
    let end_date = DateTime.fromISO(
      end_date_or_time ? rest.end.date : rest.end.dateTime
    );
    return {
      ...rest,
      ...
      type:
        start_date.toMillis() === end_date.plus({ days: -1 }).toMillis() &&
        start_date.startOf("day").toMillis() === start_date.toMillis()
          ? "day_event"
          : start_date.toMillis() === end_date.toMillis()
          ? "task"
          : start_date.startOf("day").toMillis() ===
            end_date.startOf("day").toMillis()
          ? "single_day_event"
          : "multi_day_event",
    };
  });
  items = items.sort(
    (a, b) => a.convertedStart.toMillis() - b.convertedStart.toMillis()
  );
  return items;
};
...
const convertToRecurring = (event) => {
  ...
  for (let [index, date] of dates.entries()) {
    ...
    occuranceStart = occuranceStart.setZone(start_date.zone, {
      keepLocalTime: true,
    });
    let occuranceEnd = DateTime.fromMillis(
      date.valueOf() + eventLength.valueOf()
    );
    occuranceEnd = occuranceEnd.setZone(start_date.zone, {
      keepLocalTime: true,
    });
    output.push({
      ...event,
      ...
      start: event.start.date
        ? { date: occuranceStart.toISO(), timeZone: event.start.timeZone }
        : { dateTime: occuranceStart.toISO(), timeZone: event.start.timeZone },
      end: event.end.date
        ? { date: occuranceEnd.toISO(), timeZone: event.start.timeZone }
        : { dateTime: occuranceEnd.toISO(), timeZone: event.end.timeZone },
      rrule: ruleOptions,
