...
export const convertDates = (convertedDateStart, convertedDateEnd) => {
  if (!(convertedDateStart.date instanceof DateTime)) {
    convertedDateStart.date = DateTime.fromISO(convertedDateStart.date);
  }
  if (!(convertedDateEnd instanceof DateTime)) {
    convertedDateEnd.date = DateTime.fromISO(convertedDateEnd.date);
  }

  //Same Day
  if (
    convertedDateStart.year === convertedDateEnd.year &&
    convertedDateStart.day === convertedDateEnd.day
  ) {
    //With Time
    if (convertedDateStart.time && convertedDateEnd.time) {
      ...
      if (convertedDateStart.time === convertedDateEnd.time) {
        ...
        // Not Current Year
        return (
          convertedDateStart.day +
          ", " +
          convertedDateStart.year +
          ", " +
          convertedDateStart.time
        );
        //Same AM/PM
      } else if (
        convertedDateStart.time.slice(-2) === convertedDateEnd.time.slice(-2)
      ) {
        // Current Year
        if (Number(convertedDateStart.year) === Number(DateTime.now().year)) {
          return (
            convertedDateStart.day +
            ", " +
            convertedDateStart.time.slice(0, -2) +
            "-" +
            convertedDateEnd.time
          );
        }
        // Not Current Year
        return (
    ...
    if (!convertedDateStart.time && !convertedDateEnd.time) {
      // If Single Day Event
      if (
        convertedDateStart.date.toMillis() ===
          convertedDateEnd.date.toMillis() ||
        convertedDateStart.date.toMillis() ===
          convertedDateEnd.date.plus({ days: -1 }).toMillis()
      )
        return convertedDateStart.date_text;
      return (
        convertedDateStart.date_text +
        " - " +
        convertDate(convertedDateEnd.date.plus({ days: -1 }), true).date_text
      );
    }
...
export const convertDate = (date_string, time_enabled) => {
  ...
  time = !time_enabled
    ? time.slice(-6, -3) === ":00"
      ? time.slice(0, -6) + time.slice(-2).toLowerCase()
